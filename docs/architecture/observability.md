...
- PostgreSQL `business_events` хранит первичные события;
- PostgreSQL `analytics_event_exports` хранит cursor/status по каждому событию;
- Laravel command `observability:export-business-events` отправляет батчи в
  ClickHouse `analytics_events`;
- успешные события получают `status=exported`;
- временные ошибки увеличивают `attempts`;
- после лимита попыток запись получает `status=failed` и `last_error`;
- после лимита попыток recoverable ClickHouse failures также попадают в
  `dead_letters`, где их можно увидеть в `/admin/dead-letters` и вернуть в retry
  командой `observability:retry-dead-letter`.
...
```text
montry_business_events_total{event_type,status,source,plan_code}
montry_monitored_resources_total{type,status}
montry_monitors_total{type,status,enabled}
montry_open_incidents_total{severity}
montry_subscriptions_total{plan_code,status}
montry_payments_total{provider,status,currency}
montry_queue_jobs_total{queue}
montry_http_request_duration_seconds_bucket{method,route,status_class,le}
montry_internal_api_duration_seconds_bucket{method,route,status_class,le}
montry_queue_job_duration_seconds_bucket{queue,job,status,le}
```
...
montry_poller_jobs_total{check_type,source,status}
montry_poller_check_duration_seconds_bucket{check_type,status,le}
montry_poller_result_delivery_total{check_type,status}
montry_poller_queue_buffer_used
montry_poller_queue_buffer_capacity
montry_poller_workers
...
MVP-инструментация использует W3C `traceparent` и легкий OTLP/HTTP exporter без
дополнительных SDK-зависимостей:

- Laravel создает server spans для HTTP requests;
- Laravel создает spans для queue jobs и business events;
- Laravel передает `traceparent` в Go poller через HTTP header и payload;
- Go poller создает spans для получения manual check, scheduled fetch,
  выполнения проверки и публикации результата;
- Go poller возвращает `traceparent` в Laravel result API;
- OpenTelemetry Collector принимает traces на `/v1/traces` и отправляет их в
  Tempo.
