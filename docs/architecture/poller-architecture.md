...
- DNS monitoring;
- наличие `robots.txt`;
- платные проверки `sitemap_xml`, `tcp_port`, `api_endpoint`;
- будущие Ping, Keyword/content, Redirect, RKN, Blacklist, PageSpeed, Malware checks.
...
Worker знает только registry и общий интерфейс `Checker`. Детали проверки живут в отдельных пакетах: `internal/checks/httpcheck`, `internal/checks/sslcheck`, `internal/checks/domaincheck`, `internal/checks/dnscheck`, `internal/checks/robotstxtcheck`, `internal/checks/sitemapcheck`, `internal/checks/tcpportcheck`, `internal/checks/apiendpointcheck`.

Если пакет проверки еще не реализован, registry может отдавать `StubChecker`. Stub checker должен возвращать результат с `status = skipped` или `status = unknown` и `error.code = checker_not_implemented`. Laravel сохраняет такой результат, но не открывает incidents и не отправляет аварийные уведомления.
...
- `internal/checks/dnscheck` - DNS checker for configured record lookups.
- `internal/checks/robotstxtcheck` - `robots.txt` existence checker.
- `internal/checks/sitemapcheck` - paid `sitemap.xml` existence and syntax/structure checker.
- `internal/checks/tcpportcheck` - paid TCP port reachability checker.
- `internal/checks/apiendpointcheck` - paid API endpoint checker with method, URL, timeout and expected statuses.
