...
const faq = [
    [
        'Нужно ли что-то устанавливать для мониторинга сайтов онлайн?',
        'Нет. Достаточно добавить адрес сайта или домен в личном кабинете. Montry выполняет проверку доступности сайта со своей стороны, поэтому плагины, скрипты или агенты не нужны.',
    ],
    [
        'Как быстро придут уведомления о падении сайта?',
        'Скорость зависит от интервала мониторинга на тарифе. Чтобы не тревожить из-за случайного таймаута, Montry подтверждает проблему повторными проверками и только затем открывает инцидент.',
    ],
    [
        'Будут ли уведомления после каждой неудачной проверки?',
        'Нет. Уведомление о падении сайта отправляется при открытии подтверждённого инцидента, а не после каждой ошибки. Когда сайт восстановится, вы получите отдельное сообщение.',
    ],
    [
        'Что произойдёт, когда сайт снова заработает?',
        'Montry зафиксирует восстановление, закроет инцидент и сохранит длительность простоя в истории. Так видно, когда началась проблема и сколько она продолжалась.',
    ],
    [
        'Как работает мониторинг SSL и срока домена?',
        'Сервис регулярно проверяет срок SSL-сертификата и регистрации домена и заранее предупреждает о критической дате. По умолчанию используются интервалы 30, 14, 7, 3 и 1 день.',
    ],
    [
        'Подходит ли Montry для мониторинга сайтов веб-студии?',
        'Да. Сайты клиентов можно объединять по проектам, видеть статусы, историю инцидентов и отчёты в одном кабинете — без ручных проверок по списку.',
    ],
    [
...
const jsonLd = computed(() => ([
    {
        ...
        name: 'Montry',
        url: `${appUrl.value}/`,
        logo: `${appUrl.value}/images/og-default.png`,
        description: 'Сервис мониторинга сайтов: доступность, SSL, домен и уведомления о сбоях.',
    },
    {
        '@context': 'https://schema.org',
        '@type': 'WebSite',
        ...
        url: `${appUrl.value}/`,
        inLanguage: 'ru-RU',
        description: 'Мониторинг сайтов онлайн: проверка доступности, SSL, домена и уведомления о падении сайта.',
    },
    {
        '@context': 'https://schema.org',
        '@type': 'FAQPage',
...
<template>
    <SeoHead
        title="Мониторинг сайтов онлайн"
        description="Мониторинг сайтов онлайн: проверка доступности сайта, SSL и срока домена. Уведомления о падении сайта на почту и в Telegram — для владельцев и веб-студий."
        path="/"
        :json-ld="jsonLd"
    />
    ...
    <main class="min-h-screen bg-[#F3F8F5] font-sans text-[#26332D]">
            ...
            <div class="mx-auto grid max-w-6xl gap-10 px-5 pb-16 pt-12 sm:px-8 lg:grid-cols-[minmax(0,1fr)_460px] lg:items-center lg:pb-20 lg:pt-16">
                <div>
                    ...
                    <p class="mt-5 max-w-2xl text-base leading-7 text-[#6B7D72] sm:text-lg">
                        Мониторинг сайтов онлайн: проверка доступности, мониторинг SSL и срока домена. Уведомления о падении сайта — на почту и в Telegram.
                    </p>
        ...
        <section id="about-monitoring" class="border-b border-[#DDEBE3] bg-white py-14 sm:py-16">
            <div class="mx-auto max-w-3xl px-5 sm:px-8">
                <h2 class="text-3xl font-bold leading-tight text-[#26332D] sm:text-4xl">
                    Зачем нужен мониторинг сайтов
                </h2>
                <div class="mt-6 space-y-4 text-base leading-7 text-[#52645A]">
                    <p>
                        Мониторинг сайтов помогает узнать о сбое раньше клиентов и рекламы. Montry круглосуточно выполняет проверку доступности сайта, следит за HTTP-ответом и фиксирует подтверждённые инциденты, а не каждый случайный таймаут.
                    </p>
                    <p>
                        Отдельно сервис ведёт мониторинг SSL и срока регистрации домена: заранее предупреждает, когда сертификат или домен заканчиваются. Владельцы получают уведомления о падении сайта на email, а на платных тарифах — ещё и в Telegram.
                    </p>
                    <p>
                        Для веб-студий и фрилансеров удобен единый кабинет: все клиентские проекты в одном списке, со статусами, историей инцидентов и отчётами. Подробнее о практике — в
                        <Link href="/articles" class="font-semibold text-[#24A869] transition hover:text-[#1D9059]">статьях</Link>
                        и в разделе
                        <a href="#pricing" class="font-semibold text-[#24A869] transition hover:text-[#1D9059]">тарифов</a>.
                    </p>
                </div>
            </div>
        </section>
        ...
        <section id="faq" class="py-14 sm:py-16">
            <div class="mx-auto max-w-5xl px-5 sm:px-8">
                <div class="mx-auto max-w-2xl text-center">
                    ...
                    <p class="mt-3 text-sm leading-6 text-[#738479]">Коротко о мониторинге сайтов, уведомлениях о падении, SSL и работе для веб-студий.</p>
                </div>
